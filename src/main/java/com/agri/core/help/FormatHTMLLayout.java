...
public class FormatHTMLLayout extends HTMLLayout {   
    ...
    String title=new String((format.format(new Date())+"_系统日志"));   
   ...
   if (locationInfo) {   
    ...
    public String getHeader() {   
        ...
        sbuf.append("<!--" + Layout.LINE_SEP);   
        sbuf.append("body, table {font-family: 'default',arial,sans-serif; font-size: 12px;}" + Layout.LINE_SEP);   
        ...
        if (locationInfo) {   
            sbuf.append("<th>位置</th>" + Layout.LINE_SEP);   
        }   
   
        sbuf.append("<th>原因描述</th>" + Layout.LINE_SEP);   
        sbuf.append("</tr>" + Layout.LINE_SEP);   
